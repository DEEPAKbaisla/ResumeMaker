...
const ResumeBuilder = () => {
  ...
  const saveResume = async () => {
    try {
      let updateResume = structuredClone(resumeData);

      //remove image from updated resume
      if (typeof resumeData.personal_info.image === "object") {
        delete updateResume.personal_info.image;
      }
      const formData = new FormData();
      formData.append("resumeId", resumeId);
      formData.append("resumeData", JSON.stringify(updateResume));
  ...
  return (
    <div>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <Link
          to={"/app"}
          className="inline-flex gap-2 items-center text-slate-700 transition-all">
